"""A heuristic approach could allocate the budget proportionally based on historical ROI or conversions for each channel. This will serve as a benchmark against the reinforcement learning model.

Do you want a simple rule-based allocation, like:

Proportional to past ROI (higher ROI gets more budget)
Equal split across all channels
Based on past conversions (channels with more conversions get more budget)"""
...
def heuristic_budget_allocation(data):
    ...
    budget_allocation = roi_per_channel / total_roi
